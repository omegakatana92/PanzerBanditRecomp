...
A modern fan-made recompilation project for **Panzer Bandit**, originally released for the **Sony PlayStation**.
...
To play, you must provide your **own legally obtained copy** of *Panzer Bandit*.
...
* `launcher\panzerBandit.bat` â€” double-click to start.
* `launcher\panzerBandit.ps1` â€” the actual PowerShell + WinForms GUI.

The launcher lets you:

1. Pick your legally obtained Panzer Bandit `.cue` file (Browse...).
...
`%APPDATA%\%PanzerBanditRecomp\launcher-config.json` and re-loaded the next time
the launcher starts.
...
```bash
git clone https://github.com/omegakatana92/PanzerBanditRecomp.git
cd PanzerBanditRecomp
```
...
**Panzer Bandit**, its characters, names, artwork, music, trademarks, and other intellectual property remain the property of their respective copyright and trademark holders.
