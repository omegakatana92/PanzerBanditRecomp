...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\launcher\panzerBandit.ps1
...
#     %APPDATA%\PanzerBanditRecomp\launcher-config.json (per-user). Falls back
...
[CmdletBinding()]
param(
    [string]$RecompExeName = 'Panzer_Bandit_Recompiled.exe'
)
...
function Get-ConfigDir {
    ...
    $dir = Join-Path $appData 'PanzerBanditRecomp'
...
$form.Text = 'Panzer Bandit Launcher'
...
$lblInfo.Text = "Select your legally obtained Panzer Bandit disc image (.cue).
...
$btnBrowse.Add_Click({
    ...
    $dlg.Title = 'Select your legally obtained Panzer Bandit disc image'
...
function Show-Error {
    ...
    [System.Windows.Forms.MessageBox]::Show($form, $Message, 'Panzer Bandit Launcher', 'OK', 'Warning') | Out-Null
}
